module swmc_operators
    ...
    contains
        subroutine divcorrection(u,h,div,dt,mesh)
            ...
            type(scalar_field) :: u_ed,grad,ugrad,h_inv,udiff! scalar at cells
        ...
        subroutine div_ed(f_ed,div,mesh)
            ...
            real*8  :: pp(3),pu(3),A,le,nr(3)
            integer :: i,j,k,l,sh,signcor
    ...
    !--------------------------------------------
    subroutine calcop(u,h,bath,uh,h_ed,zeta,q,div,uhq_perp,gradb, &
            ke,nu,lap,bih,f,fv,mesh,dt)
      implicit none
      type(grid_structure), intent(in) :: mesh
      type(scalar_field), intent(in)   :: f,fv,u,h,bath
      type(scalar_field), intent(inout):: div,gradb,uhq_perp,h_ed,q,zeta, &
          uh,lap,bih
...
!      call visc(lap,nu2,bih,mesh);
    ...
    subroutine calcophx(u,h,uh,h_ed,bath,zeta,q,div,uhq_perp,grad,ke,nu,lap,bih,ft,mesh,dt)
      implicit none
      type(grid_structure), intent(in) :: mesh
      type(scalar_field), intent(in)   :: ft,u,h,bath
      type(scalar_field), intent(inout):: div,grad,uhq_perp,h_ed,q,zeta,uh,lap,bih
      type(scalar_field)               :: ke,h_tr,q_ed,eta,phi
      type(vector_field_cart)          :: uhq_v
      real*8,intent(in) :: dt,nu
      real*8 :: nu2

      allocate(h_tr%f(mesh%nt), &
        uhq_v%p(mesh%nv),eta%f(mesh%nv), &
        q_ed%f(mesh%ne),phi%f(mesh%nv));
      ...
      call grad_edhx(phi, grad, mesh);
      !------------------------------------

      !------------------------------------
      call scalar_ve2tr(h, h_tr, mesh);
      ! zeta%f = 0d0;
      ! h_tr%f = 1d0;
      eta%f = zeta%f+ft%f;
      ...
      if (.true.) then 
        call scalar_tr2ed(q, q_ed, mesh); ! arith mean
      else
        call apvm(u, q,dt, q_ed, mesh) ! antecipated pot vort
      endif
      !------------------------------------
      ...
      !-Div--------------------------------
      call scalar_ve2ed(h, h_ed, mesh);
      ! h_ed%f = 1d0;
      uh%f = u%f*h_ed%f;
      call div_hx(uh, div, mesh);
      !------------------------------------

      !-Coriolis Term----------------------
      call coriolis_edhx(uh, q_ed, uhq_perp, mesh);
      ! uhq_perp%f = uhq_perp%fexact;
      ! ------------------------------------
      ! uhq_perp%f =0d0;

      if (abs(nu)>1d-15) then
        call vischx(u,nu,lap,mesh);
        nu2=1d0;
        call vischx(lap,nu2,bih,mesh);
      endif

    end subroutine calcophx
    !--------------------------------------------
    ...
    subroutine errorop(u,utr,h,zeta,q,div,uperp,gradb,ke,fv,mesh,dt,file)
      ...
      real*8                              :: nu
      real*8                              :: varq(mesh%nv,2),varp(mesh%nt,2),vare(mesh%ne,2),vecp(mesh%nt,3,2)
      type(scalar_field)                  :: gradk,f
      ...
      phi%f = g*(bath%f+h%f);
      call calcop(u,h,bath,uh,h_ed,zeta,q,div,uhq_perp,gradb,ke,nu,lap,bih,f,fv,mesh,dt)
      ...
      write(files,'(2A)') trim(file),'polyharm.dat';
    ...
    subroutine tenvar(dTdt,uh,gradb,div,h,mesh)
      ...
      dTdt = sum(Ae*uh%f*gradb%f)-sum(A*h%f*div%f);
